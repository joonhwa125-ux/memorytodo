...
import type { ApiIntent } from "@/lib/api/types";
    ...
    <main className="clean">
      <div className="clean-container">
        <header style={{ marginBottom: 24 }}>
          <div style={{ fontSize: 12.5, color: "var(--c-ink-3)", fontWeight: 500, marginBottom: 6 }}>
        ...
        <section className="clean-card" style={{ marginBottom: 12 }}>
          {loading ? (
            <div style={{ padding: "20px 0", color: "var(--c-ink-3)", fontSize: 14, textAlign: "center" }}>
              불러오는 중...
            </div>
          ) : intents.length === 0 ? (
            <div style={{ padding: "30px 0", textAlign: "center" }}>
              <p style={{ color: "var(--c-ink-2)", fontSize: 15, lineHeight: 1.6, margin: 0, marginBottom: 12 }}>
                아직 마음에 둔 일이 없어요.
              </p>
              <Link href="/intents/new" className="clean-link">+ 첫 할 일 추가하기</Link>
            </div>
          ) : (
            <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
              {intents.map((intent) => (
                <li key={intent._id} className="clean-row">
                  <span className={intent.intentType === "do" ? "clean-tag-do" : "clean-tag-avoid"}>
                    {intent.intentType === "do" ? "할 일" : "참기"}
                  </span>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 15, fontWeight: 500, color: "var(--c-ink)", lineHeight: 1.4 }}>
                      {intent.title}
                    </div>
                    {intent.why && (
                      <div style={{ fontSize: 12, color: "var(--c-ink-3)", marginTop: 4, lineHeight: 1.4 }}>
                        {intent.why}
                      </div>
                    )}
                    {intent.dueDate && (
                      <div style={{ fontSize: 11.5, color: "var(--c-accent)", marginTop: 4, fontWeight: 600 }}>
                        마감 {intent.dueDate}
                      </div>
                    )}
                  </div>
